...
export const saleService = {
    ...
    try {
      ...
      return {
        ...
        message:
          ...
          "Error fetching customer's sales data",
      };
    }
  },
  getUser: async (_id = "") => {
  ...
  checkInventory: async (optimizeProdId, optimizeStoreId = "") => {
    try {
      ...
      const response = await api.get(SALE_ENDPOINTS.CHECK_INVENTORY, {
        params,
      });
      return {
        success: true,
  ...
  getSalesRep: async (storeId) => {
    try {
      ...
      return {
        success: false,
        message:
          error.response?.data?.message || "Error fetching salesRep data",
      };
    }
  },
  ...
  uploadFile: async (formData) => {
    try {
      const response = await api.post(SALE_ENDPOINTS.FILE_UPLOAD, formData, {
        headers: {
  ...
  addSales: async (data) => {
    try {
      const response = await api.post(SALE_ENDPOINTS.SALES, data);
      return {
        success: true,
        data: response.data,
      };
