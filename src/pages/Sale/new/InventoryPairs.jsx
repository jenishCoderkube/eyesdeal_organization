...
export default function InventoryPairs({
  ...
  const normalizeForPair = (prod) => {
    ...
    return {
      ...
      discount,
      totalAmount,
      quantity: prod.quantity ?? prod?.inventory?.totalQuantity ?? 0,
      raw: prod,
    };
  };
  ...
  const handleSlotChange = async (pairId, slot, option) => {
    ...
    // if (!inv) return;
    const normalized = normalizeForPair(option?.data);
    ...
    if (slot === "leftLens") {
      ...
      updatePair(pairId, {
        leftLens: normalized,
        rightLens: normalized,
        autoFilledRight: true,
      });
      return;
    }
          ...
          <div key={pair.pairId} className="mb-4">
            ...
            <div className="table-responsive">
              <table className="table table-sm table-striped">
                ...
                <tbody>
                  {pair.product &&
                    renderRow("Frame", pair.product, pair.pairId, "product")}
                  {pair.leftLens &&
                    renderRow(
                      "Left Lens",
                      pair.leftLens,
              ...
              {!pair.product && (
                <div style={{ flex: 1 }}>
                  ...
                  <AsyncSelect
                    ...
                    onChange={(opt) =>
                      handleSlotChange(pair.pairId, "product", opt)
                    }
                  />
                </div>
              )}
              ...
              {!pair.leftLens && (
                <div style={{ flex: 1 }}>
                  ...
                  <AsyncSelect
                    ...
                    onChange={(opt) =>
                      handleSlotChange(pair.pairId, "leftLens", opt)
                    }
                  />
                </div>
              )}
              ...
              {!pair.rightLens && (
                <div style={{ flex: 1 }}>
                  ...
                  <AsyncSelect
                    ...
                    onChange={(opt) =>
                      handleSlotChange(pair.pairId, "rightLens", opt)
                    }
                  />
                </div>
              )}
