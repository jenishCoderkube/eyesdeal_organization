...
export default function InventoryData({ inventoryData, setInventoryData, setInventoryPairs }) {
    ...
    useEffect(() => {
        const updatedData = inventoryData.map((item) => {
            ...
            const { taxAmount, discount, totalAmount } = calculateInvoiceValues(mrp, srp, taxRate);
            return {
                ...item,
                taxAmount,
                discount,
                            ...
                            if (item.type === 'product' || item.type === 'lens') {
                                ...
                                return (
                                    <tr key={`item-${index}`}>
                                        ...
                                        <td className="custom-td">{isProduct ? data?.sku : data?.productName}</td>
                                        <td className="custom-td">
                                            {data?.photos?.length > 0 ? (
                                                <img src={data?.photos} alt="Product" style={{ width: 40, height: 40 }} />
                                            ) : (
