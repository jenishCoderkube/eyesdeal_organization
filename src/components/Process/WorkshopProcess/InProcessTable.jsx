...
const InProcessTable = () => {
        ...
        Object.values(item).some((field) => {
          if (typeof field === "object" && field !== null) {
            return Object.values(field).some(
              (subField) =>
                subField &&
                subField.toString().toLowerCase().includes(lowerQuery)
            );
          }
          return field && field.toString().toLowerCase().includes(lowerQuery);
        })
      );
    },
    []
  ...
  useEffect(() => {
    const debouncedFilter = debounce((query) => {
      setFilteredData(filterGlobally(tableData, query));
    }, 200);
    debouncedFilter(searchQuery);
    return () => clearTimeout(debouncedFilter.timeout);
  }, [searchQuery, tableData, filterGlobally]);
        ...
        cell: ({ getValue }) => (
          <div className="table-vendor-data-size">
            <div className="d-flex flex-column">
              <span className="text-danger">Right: {getValue().right}</span>
              <span className="text-danger">Left: {getValue().left}</span>
              <button
                className="btn btn-outline-primary border-light-subtle mt-2"
                type="button"
                style={{ color: "#6366f1" }}
                onClick={() => handleDownloadPhoto(getValue())}
              >
                Download Photo
              </button>
            </div>
